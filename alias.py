...
def create_parser():
	parser = argparse.ArgumentParser(description='Enables an easy way to add and remove an alias for the gwiz python program. The alias will allow the user to call the program from anywhere in their user directory tree by simply typing "gwiz [arguments]".', epilog='This script edits the .bash_profile of the current user. It simply adds, updates, or removes the following line to manage the alias; alias gwiz="python3 absolute_path_to_gwiz.py". The current working directory is used to find the absolute file path of gwiz.py and can not be done in an efficient manner otherwise.')
	parser.add_argument('-s', '--show_file', action='store_true', help='Prints the contents of .bash_profile after it has been edited.')
	group = parser.add_mutually_exclusive_group()
	group.add_argument('-c', '--create', action='store_true', help='Creates the alias. The gwiz.py file must be in the current working directory.')
